...
- **CC-001**: Requirements for this feature MUST be captured in `project-spec/` numbered files
  before implementation starts.
- **CC-002**: Implementation paths MUST map to `apps/web` or
  `packages/{schema, ai, database, integrations, ui}`.
- **CC-003**: Runtime data validation MUST use Zod schemas (prefer shared schemas from
  `packages/schema`).
